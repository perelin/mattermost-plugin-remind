...
- Go (1.23+) on PATH
- git on PATH
- `.env` file in repo root with credentials (see below)

## Step 1: Create `.env` File

Create `.env` in the repo root (already in `.gitignore`, won't be committed):
...
2. Builds Go server binaries (multi-arch: linux, darwin, windows)
3. Bundles everything into `dist/<plugin-id>-<version>.tar.gz`
4. Uploads the bundle to the server via UploadPluginForced API
5. Enables the plugin via EnablePlugin API
...
All commands need `source .env` first unless the vars are already exported.
...
| Error                               | Cause / Fix                                                    |
|-------------------------------------|----------------------------------------------------------------|
| `MM_SERVICESETTINGS_SITEURL is not set` | Run `source .env` before the command                       |
...
- Credentials live in `.env` (gitignored) — never commit secrets to the repo
